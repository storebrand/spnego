...
public final class SpnegoHttpFilter implements Filter {
    ...
    private final transient List<String> excludeDirs = new ArrayList<>();
        ...
        try {
            ...
            if (!props.getProperty("spnego.authz.class", "").isEmpty()) {
                ...
                this.accessControl = (UserAccessControl) Class.forName(
                        props.getProperty("spnego.authz.class")).getConstructor().newInstance();
                this.accessControl.init(props);
            }

        }
        catch (final LoginException | GSSException | PrivilegedActionException | FileNotFoundException |
                     URISyntaxException | ReflectiveOperationException lex) {
    ...
    private boolean isAuthorized(final HttpServletRequest request) {
        return null == this.sitewide || null == this.accessControl
    ...
    public static final class Constants {
         ...
         *
         * <p>Flag to indicate if requests coming from <a href="http://localhost">...</a>
